...
## 단계별로 보면 좋은 파일·함수 (레포 기준)

경로는 저장소 루트 기준이다. `static` 함수는 해당 `.c` 안에서만 정의된다.

| 단계 | 볼 파일 | 볼 함수·심볼 |
| --- | --- | --- |
| 선행 | `docs/01-product-planning.md` ~ `04-development-guide.md`, `docs/weeks/WEEK7/sequences.md` | (문서 위주) |
| 1 | `include/week7/bplus_tree.h`, `src/week7/bplus_tree.c`, `tests/test_bplus_tree.c` | 공개 API: `bplus_create`, `bplus_destroy`, `bplus_insert`, `bplus_search`, `bplus_insert_or_replace` · 내부: `find_leaf`, `leaf_insert_sorted`, `leaf_split_insert`, `internal_split_insert`, `insert_into_parent`, `node_new`, `node_free` |
| 2 | `include/week7/week7_index.h`, `src/week7/week7_index.c`, `include/csv_storage.h`, `src/csv_storage.c` | `week7_reset`, `week7_ensure_loaded`, `week7_table_has_id_pk`, `week7_prepare_insert_values`, `week7_free_prepared` · `csv_storage_data_row_count`, `csv_storage_column_count`, `csv_storage_read_table`, `csv_storage_free_table` |
| 3 | `src/executor.c`, `docs/weeks/WEEK7/sequences.md` | `executor_execute_insert`, `week7_on_append_success` (append 직후 `row_index` 반영) |
| 4 | `src/parser.c`, `include/parser.h`, `src/lexer.c`, `include/lexer.h`, `include/ast.h`, `src/ast.c`, `tests/test_parser_select.c` | `parser_parse_select` · `ident_is_id` · `lexer_next` 및 `TOKEN_WHERE`, `TOKEN_EQ` · `SelectStmt.has_where_id_eq`, `where_id_value` · `ast_select_stmt_free` |
| 5 | `src/executor.c`, `include/week7/week7_index.h`, `src/week7/week7_index.c`, `src/csv_storage.c` | `executor_execute_select` · `week7_ensure_loaded`, `week7_table_has_id_pk`, `week7_lookup_row` → 내부에서 `bplus_search` · `csv_storage_read_table`, `csv_storage_free_table` |
| 6 | `docs/03-api-reference.md`, `src/sql_processor.c`, `src/main.c`, `tests/test_executor.c`, `tests/test_parser_select.c` | `sql_processor_run_file`(종료 코드·실행 루프) · `test_select_where_hit`, `test_select_where_miss` 등 |
| 7 | `src/bench_bplus.c`, 루트 `README.md` | `main`, `run_full`, `run_compare`, `prng` |

같은 문서 뒤쪽 **「한눈에: 단계 ↔ 이 레포 파일」** 표와 함께 쓰면 된다.
...
**함수·심볼 단위**로 보려면 위쪽 **「단계별로 보면 좋은 파일·함수 (레포 기준)」** 표를 본다.
