...
- 외부 요청은 API 서버가 받는다.
- 병렬 처리는 Thread Pool과 Job Queue가 담당해서 동시성 이슈를 해결하고자 함.
- 내부 DB 엔진과 외부 API 서버 사이 연결 설계는 HTTP 요청을 SQL 실행으로 넘기고 결과를 JSON 응답으로 바꾸는 Engine Bridge 계층에서 설계함.
...
이 비교는 어떤 방식이 더 많은 요청을 받아들이는지보다, 요청이 몰렸을 때 서버가 얼마나 안정적으로 버티고, 어디서부터 거절하기 시작하는지를 보기 위한 것이다.
...
다음은 요청이 많이 몰릴 때, 시간 제한을 고정해 두고 바로 거절하는 방식과 상황에 따라 기다리는 시간을 조절하는 방식을 비교한 것이다. 
즉, 서버가 바쁠 때 어떤 방법이 더 안정적으로 대응하고, 결과를 더 쉽게 설명할 수 있는지 본 것이다.
